...
function App() {
    ...
    const setupListeners = async () => {
      const unlistenToken = await listen<string>(
        ...
        (event) => {
          ...
          if (elapsed > 0.5) {
            // Update every 500ms roughly, or just calc
  ...
  const drawVisualizer = () => {
    ...
    // Draw "Pulse"
    ctx.lineWidth = 2;
    ctx.strokeStyle = "#2ae876"; // Plexus Green
    ctx.shadowBlur = 10;
  ...
  const startRecording = async () => {
    try {
      ...
      // Create Context (try 16k for Whisper compatibility, browser resamples)
      const ctx = new (
  ...
  return (
    <div className="min-h-screen bg-plexus-dark text-plexus-text p-8 font-sans selection:bg-plexus-cyan/30 selection:text-white overflow-hidden relative">
      {/* Mesh Map Modal */}
      {showMeshMap && (
      ...
      {/* Background Ambience - Logo Colors */}
      <div className="absolute top-[-20%] left-[-20%] w-[60%] h-[60%] bg-plexus-green/5 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-plexus-blue/10 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute top-[30%] left-[40%] w-[40%] h-[40%] bg-plexus-cyan/5 rounded-full blur-[90px] pointer-events-none"></div>

      <div className="max-w-5xl mx-auto space-y-8 relative z-10">
        {/* Header / Status Bar */}
        <div className="flex justify-between items-center p-6 bg-plexus-card backdrop-blur-xl border border-plexus-border rounded-2xl shadow-2xl">
          <div className="flex items-center space-x-5">
            {/* Logo Icon */}
            <div className="w-12 h-12 relative flex items-center justify-center">
              <div className="absolute inset-0 bg-logo-gradient opacity-20 blur-md rounded-full"></div>
              <img
                src="/logo.png"
                alt="Plexus Logo"
                className="w-10 h-10 drop-shadow-lg object-contain rounded-full"
              />
            </div>
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-white to-plexus-muted">
                PLEXUS
              </h1>
              <div className="flex items-center space-x-2">
                <div className="h-[2px] w-8 bg-logo-gradient rounded-full"></div>
                <p className="text-plexus-muted text-[10px] tracking-[0.2em] uppercase font-mono">
                  Neural Mesh
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center space-x-8 bg-plexus-dark/50 px-6 py-3 rounded-xl border border-plexus-border/50">
            {/* Settings Button */}
            <button
            ...
            <div className="text-right">
              <p className="text-[10px] text-plexus-muted uppercase tracking-widest font-semibold mb-1">
                Identity
              </p>
              <div className="font-mono text-xs text-plexus-cyan">
                {status
                  ? status.peer_id.substring(0, 12) + "..."
                  : "Initializing..."}
              </div>
            </div>
